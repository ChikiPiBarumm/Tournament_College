...
class RegistrationApp:
    ...
    def create_widgets(self):
        ...
        self.button_individual = tk.Button(individual_team_frame, text="As Individual", command=self.register_individual)
        ...
        self.button_complete = tk.Button(self.master, text="Complete Registration", command=self.show_tournament_handling_window)
    ...
    def create_exit_help_buttons(self, parent, help_message):
        exit_help_frame = tk.Frame(parent)
        exit_help_frame.pack(side=tk.BOTTOM, fill=tk.X, pady=10)  # Padding at the bottom

        self.button_exit = tk.Button(exit_help_frame, text="Exit", command=self.confirm_exit)
    ...
    def submit_team(self):
        ...
        self.cursor.execute("SELECT TeamID FROM Teams WHERE TeamName=?", (team_name,))
        existing_team = self.cursor.fetchone()
        if existing_team:
            messagebox.showerror("Error", "Team name must be unique.")
            return
    ...
    def show_tournament_handling_window(self):
        ...
        for event in individual_events:
            ...
            tree = ttk.Treeview(individual_event_tab, columns=("Rank", "ID", "Participant", "Points"), show="headings",
                                style="Treeview")
            ...
            tree.bind('<Double-1>',
                      lambda event, tree=tree, event_id=event_id, event_type='Individual': self.edit_points(event, tree,
                                                                                                            event_id,
                                                                                                            event_type))
        ...
        for event in team_events:
            ...
            tree.bind('<Double-1>',
                      lambda event, tree=tree, event_id=event_id, event_type='Team-based': self.edit_points(event, tree,
                                                                                                            event_id,
                                                                                                           event_type))
